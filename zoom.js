...
function InitWebGL()
{
	...
	LoadMeshAndSetup(rock1, objrock1, imgrock1);
	...
	LoadMeshAndSetup(rock7, objrock5, imgrock2);
...
function DrawScene()
{
	...
	for (var i = 0; i< fishDrawers.length; i++){
		...
		var mv  = GetModelViewMatrix( tran[i][3], tran[i][0],  tran[i][1]+ translateY, transZ, 0.0,  autorot + rotY );
		var mvp = MatrixMult( perspectiveMatrix, mv );
		var nrmTrans = [ mv[0],mv[1],mv[2], mv[4],mv[5],mv[6], mv[8],mv[9],mv[10] ];
...
// Funzione per caricare mesh e impostare texture
function LoadMeshAndSetup(drawer, objUrl, textureUrl) {
    // Carica il file OBJ specificato dall'URL passato come parametro
    drawer.mesh.load(objUrl, () => {
        // Una volta caricato e analizzato il file OBJ
...
function NewScene()
{
    ...
    gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT);  // Include anche il buffer di profondità se utilizzato
